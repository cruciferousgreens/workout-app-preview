    ...
    function workoutPRs(workout) {
      ...
      workout.exercises.forEach(item=>{
        ...
        const priorSets=workoutState.completed.filter(row=>row.id!==workout.id&&row.date<workout.date).flatMap(row=>row.exercises.filter(entry=>entry.exerciseId===item.exerciseId).flatMap(entry=>entry.sets)).filter(set=>Number(set.w)>0);
    ...
    function completedExerciseMarkup(item) {
      ...
      return `<section class="completed-exercise-detail"><div class="completed-exercise-detail-head"><button class="completed-exercise-link" type="button" data-id="${escapeHtml(item.exerciseId)}">${escapeHtml(ex?.name||'Exercise')}</button><span>${tracking==='time'?`${item.sets.reduce((n,set)=>n+(Number(set.seconds)||0),0)} sec total`:`${formatVolume(volume)}${isBodyweight&&!volume?' · bodyweight':''}`}</span></div>${item.exerciseTags?.length?`<div class="exercise-tag-row">${item.exerciseTags.map(tag=>`<span class="exercise-tag-chip ${workoutState.exerciseTagPresets.includes(tag)?'preset':''}">${escapeHtml(tag)}</span>`).join('')}</div>`:''}<div class="completed-set-head"><span>SET</span><span>${tracking==='time'?'WEIGHT / TIME':'WEIGHT × REPS'}</span><span>RPE</span><span>EST. 1RM</span></div>${item.sets.map((set,index)=>{const estimate=tracking==='reps'&&Number(set.w)>0?Math.round(estimate1RM(set)):null;const load=isBodyweight?(Number(set.w)>0?`+${set.w} lb`:'Bodyweight'):`${set.w??'—'} lb`;const performance=tracking==='time'?`${load} · ${set.seconds??'—'} sec`:`${load} × ${set.r??'—'}`;return `<div class="completed-set-row"><span class="set-num">${index+1}</span><span class="completed-set-cell" data-label="${tracking==='time'?'Weight / time':'Weight × reps'}"><strong>${escapeHtml(performance)}</strong></span><span class="completed-set-cell completed-rpe" data-label="RPE">${set.rpe==null?'—':`<strong>${set.rpe}</strong>`}</span><span class="completed-set-cell completed-est" data-label="Est. 1RM">${tracking==='time'?'—':estimate?`${estimate} lb`:'—'}</span>${set.tags?.length?`<div class="completed-set-tags">${set.tags.map(tag=>`<span class="set-tag-chip">${escapeHtml(tag)}</span>`).join('')}</div>`:''}</div>`}).join('')}${item.note?`<p class="completed-note"><strong>Notes:</strong> ${escapeHtml(item.note)}</p>`:''}</section>`;
    }
