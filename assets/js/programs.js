...
/* ===== module: programs.js ===== */
    ...
    function applyRepPreset(key,target=progressionSetup){const preset=REP_PRESETS[key]||REP_PRESETS.hypertrophy;target.defaultRange={preset:key,min:preset.min,max:preset.max,openTop:!!preset.openTop,amrap:!!preset.amrap};if(target===progressionSetup){$('#programRepMin').value=preset.min;$('#programRepMax').value=preset.max;const settingsMin=$('#settingsRepMin'),settingsMax=$('#settingsRepMax');if(settingsMin)settingsMin.value=preset.min;if(settingsMax)settingsMax.value=preset.max;document.querySelectorAll('[data-rep-preset]').forEach(button=>button.setAttribute('aria-pressed',String(button.dataset.repPreset===key)));}}
    ...
    function startWorkoutFromTemplate(id) {
      ...
      prepareDraftProgression(workoutState.draft,freeformProgressionConfig());schedulePersist();renderWorkoutScreen();
    }
